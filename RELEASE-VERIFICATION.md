...
| Activity UI contract | live desktop investigation plus source contract | stationary detective scan showed real callback text; droid search, lens pulse, scan beam, and three sequential evidence-node animations were active only while work was active; no separate `Preparing investigation` screen or fabricated countdown remained; reduced-motion CSS disables the animations |
...
- The UI shows actual investigation and sandbox milestones rather than a silent spinner or
  fabricated countdown. The stationary detective scan is active only during real investigation work.
